...
from PyQt5.QtWidgets import (
    ...
    QGridLayout, QFrame, QSpinBox, QMessageBox
)
from PyQt5.QtCore import Qt, QTimer
from PyQt5.QtGui import (
    QPainter, QColor, QBrush, QPen, QIntValidator, QFont, QIcon
)
...
class Metronome(QWidget):
    ...
    def start(self):
        # 无音频时提示用户（一次性）
        if not HAS_SOUND and not getattr(self, "_warned_no_sound", False):
            self._warned_no_sound = True
            QMessageBox.warning(
                self, "无音频",
                "未能加载音频库，将以静音模式运行（视觉节拍仍可用）。\n\n"
                f"原因：{SOUND_ERROR}\n\n"
                "请在命令行运行：pip install sounddevice cffi")
        ...
        if not self.engine.start(self._sub_interval()):
            if HAS_SOUND and self.engine.last_error \
                    and not getattr(self, "_warned_dev_err", False):
                self._warned_dev_err = True
                QMessageBox.warning(
                    self, "音频设备错误",
    ...
    def apply_theme(self):
        ...
        self.setStyleSheet(f"""
            ...
            #miniBpm {{
                ...
                background: transparent; border: none; min-width: 62px;
            }}
            #miniBpm:focus {{ background: {th['hover']}; border-radius: 6px; }}
            #miniUnit {{
                font-size: 9px; color: {th['sub']};
                background: transparent; letter-spacing: 1px; padding-bottom: 4px;
            }}
            #miniStep {{
...
if __name__ == "__main__":
    # Windows 任务栏图标修复：声明独立的应用 ID
    try:
        import ctypes
        ctypes.windll.shell32.SetCurrentProcessExplicitAppUserModelID(
            "mycompany.metronome.1.0")
    except Exception:
        pass

    QApplication.setAttribute(Qt.AA_EnableHighDpiScaling, True)
    app = QApplication(sys.argv)
    app.setWindowIcon(QIcon(resource_path("icon.ico")))   # 应用级图标
